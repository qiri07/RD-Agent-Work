...
def main():
    ...
    print(f"  从 {len(ic_unique)} 个独立因子中，筛选出 {len(selected_fids)} 个顶级因子:")
    for fid in selected_fids:
        fname = str(selected_names.get(fid, fid))[:25]
        print(f"    {fid[:12]:<14} {fname:<27} IC={selected_ic[fid]:+.4f}  pos={selected_pos[fid]:.3f}")
    ...
    print(f"  价格数据: {len(prices):,} 条 (已过滤北交所)")
    ...
    for stock in top_stocks.index:
        ...
        print(f"    {stock}: score={top_stocks.loc[stock, 'score']:.4f}  [{top3}]")
    ...
    print(perf_analyzer.generate_report(metrics, "高胜率因子选股策略 v2"))
    ...
    print(f"  指标: {OUT / 'high_winrate_metrics.json'}")
