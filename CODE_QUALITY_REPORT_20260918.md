...
| I001 | `scripts/buffett_daily_analysis.py` | :21-23 | 导入块未排序（`_get_stock_name` 应在 `analyze_stock` 前） |
| I001 | `tests/test_daily_analysis.py` | :10-17 | 导入块未排序（`_extract_json` 应在 `_get_whitelist` 前） |
...
2. `test_kronos_runner_inference` 三个测试: 检查 `predictor.py` 的 `predict_batch` 方法最新实现，更新 mock 断言方式（当前可能因为预测逻辑内部变化导致 predict 未被直接调用）
...
| 现代类型语法 (`X \| None`, `list[str]`) | ✅ 未发现旧式 `Optional[]` / `List[]` 用法（.venv 除外） |
...
| 规则 | 检查结果 |
|------|----------|
| 禁止 `Any`（需注释说明） | ⚠️ `trade_krono_cli/adapters/base.py` 及 `batch_runner.py` 中有 `Any` 使用，属于合理场景（适配器动态接口、泛型批处理），但缺少注释说明 |
| 禁止 `# type: ignore`（需附错误码与理由） | ⚠️ 发现多处 `# type: ignore[...]`，部分附带了错误码（如 `# type: ignore[arg-type]`），但 `scripts/daily_analysis.py:31` 的 `# type: ignore[import-not-found]` 和 `scripts/daily_analysis.py:67` 的 `# type: ignore[misc,assignment]` 理由不充分 |
| 禁止 `except Exception as e:` 后无具体处理 | ⚠️ 发现 20+ 处 `except Exception as e:` 使用，均有 logging 或 re-raise，但未全部满足 AGENTS.md "附带上下文（ticker、日期、错误码）"的要求 |
| scripts 目录 | ⚠️ `scripts/__init__.py` 缺少 `from __future__ import annotations`；部分脚本有 `# type: ignore[import-not-found]` 绕过 import 检查 |
...
| 检查项 | 结果 |
|--------|------|
| `print()` 在库代码中 | ⚠️ 发现多处 `print()` 在 `analyze_factors.py` 等脚本中（脚本类代码允许，非库代码） |
| `os.path.join` | ⚠️ `RD-Agent/rdagent/utils/env.py` 等多处使用（这是独立子项目，不受 trade-krono-cli AGENTS.md 约束） |
| Python 版本 | Python 3.14.4（系统），.venv 中为 3.12.13 |
...
2. **`tests/test_kronos_runner_inference.py` 中 3 个测试失败**  
   ...
   - 修复: 更新 mock 断言以匹配新的预测执行路径（`predict` 可能已被内部封装调用）

### 🟡 P1 - Lint 问题（影响代码整洁）

3. **`backtest_kronos.py:8`** — `from __future__ import (...)` 括号格式问题  
...
6. **`tests/test_daily_analysis.py:10-17`** — 导入排序问题  
